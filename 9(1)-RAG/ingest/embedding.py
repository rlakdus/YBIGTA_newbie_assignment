...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    with ThreadPoolExecutor(max_workers=len(keys)) as executor:
        futures = []
        for i, (idx, batch_text) in enumerate(batches):
            client = OpenAI(api_key=keys[i % len(keys)], base_url=BASE_URL)
            futures.append(executor.submit(_embed_batch_safe, client, batch_text))
            
        for i, future in enumerate(tqdm(as_completed(futures), total=len(futures), desc="Embedding Passages")):
            batch_result = future.result()
            start_idx = i * BATCH_SIZE
            results[start_idx : start_idx + len(batch_result)] = batch_result

    embeddings_array = np.array(results, dtype=np.float32)
